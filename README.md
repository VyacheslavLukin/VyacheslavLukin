# Hi, I'm Vyacheslav 👋

📍 **Cambridge, UK** | 🤖 **Fractional CTO & AI Agent Builder** | 🚀 **Ex-Startup Founder, Carnegie Mellon MSIT-SE**

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Svelte](https://img.shields.io/badge/-Svelte-FF3E00?style=flat-square&logo=svelte&logoColor=white)
![GCP](https://img.shields.io/badge/-Google_Cloud-4285F4?style=flat-square&logo=googlecloud&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![Redis](https://img.shields.io/badge/-Redis-DC382D?style=flat-square&logo=redis&logoColor=white)
![LangChain](https://img.shields.io/badge/-LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white)
![FastAPI](https://img.shields.io/badge/-FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Cursor](https://img.shields.io/badge/-Cursor-000000?style=flat-square&logo=cursor&logoColor=white)

> From launching operator-grade telecom networks to building AI agents — 20 years mastering the full technology stack. I help startups ship 2-3x faster with AI-assisted development and strategic tech leadership.

## What I'm Building

- 🤖 **[YAi Digital](https://yai.digital)** — Co-founder & CTO. Built 6 production AI agents for content assessment and optimization. Full-stack architecture with CI/CD automation enabling continuous delivery.
- 🧑‍💻 **Fractional CTO** — Helping AI-first SaaS startups accelerate product development. Teaching teams Cursor/Claude workflows, building infrastructure that scales, and shipping features without expanding headcount.

## Career Highlights

- 🏗️ **Built 6 production AI agents** — Content assessment, optimization, and quality assurance at [YAi Digital](https://yai.digital)
- 📈 **Scaled ITS startup to $500k revenue** — Founded [Edge Vision](http://edgevision.pro.tilda.ws/), building EdgeAI software to retrofit conventional traffic signals into smart I2X-ready intersections. Real-time computer vision on the edge for traffic flow digitization, GHG reduction, and V2X-connected transport. Built from scratch with 3-person team and $120k investment
- 📡 **Launched first LTE network in Russia** — Led core network engineering at MegaFon, zero SLA breaches across millions of subscribers
- 🎓 **Carnegie Mellon MSIT-SE** — Master of Science in Information Technology - Software Engineering
- 🏆 **"Industry Leader" award** — Recognized by Innopolis University for bridging industry and academia
- 🌍 **IRF Startup Challenge semi-finalist** — International Road Federation conference presenter

## Tech Stack

| Domain | Technologies |
|---|---|
| **AI & LLM** | LangChain, LangGraph, Google ADK, Vertex AI, PyTorch, TensorFlow, Computer Vision |
| **Backend** | Python, FastAPI, Node.js, PostgreSQL, Redis, SQLite |
| **Frontend** | React, Svelte, TypeScript, JavaScript |
| **Infrastructure** | GCP, Docker, CI/CD, Linux, Terraform |
| **AI-Assisted Dev** | Cursor, Claude, Agentic Engineering |
| **Telecom (Legacy)** | MSC, MGW, HLR, HSS, STP, LTE Core, Oracle |

## What I'm Doing

- 🔧 **Building AI agents** — Designing scalable agent architectures with LangChain, LangGraph, and Google ADK
- 🚀 **Accelerating startups** — Fractional CTO engagements helping pre-seed and seed-stage founders ship faster
- 🎓 **Teaching** — Software engineering at Master's level, AI-assisted development workflows for engineering teams

## Education

- 🎓 **Carnegie Mellon University** — MSIT-SE (Master of Science in Information Technology - Software Engineering)
- 🏫 **Innopolis University** — Teaching Assistant & Senior Teacher (Software Engineering)
- 🏫 **UCL School of Management** — Teaching Assistant

## Connect

[![Website](https://img.shields.io/badge/-lukin.uk-FF5722?style=flat-square&logo=google-chrome&logoColor=white)](https://lukin.uk)
[![LinkedIn](https://img.shields.io/badge/-Vyacheslav_Lukin-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/slav-lukin/)
[![Email](https://img.shields.io/badge/-slava@yai.digital-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:slava@yai.digital)
[![YAi Digital](https://img.shields.io/badge/-YAi_Digital-000000?style=flat-square&logo=data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIyNCIgaGVpZ2h0PSIyNCIgZmlsbD0id2hpdGUiPjx0ZXh0IHg9IjIiIHk9IjE4IiBmb250LXNpemU9IjE2Ij7wn6SFPC90ZXh0Pjwvc3ZnPg==&logoColor=white)](https://yai.digital)

---

### Philosophy

> "Ship beats perfect" — I combine 20 years of systems engineering discipline with startup speed. Clean architecture, pragmatic tradeoffs, and AI-assisted workflows to move fast without breaking things.

<details>
<summary>Random Facts</summary>

- 🌍 Worked across Russia, UAE, and UK
- 📡 Started career configuring telecom switches serving millions of subscribers
- 🧑‍🏫 Created hackathon programs for children aged 10-17
- ☕ Powered by British tea and Cambridge cycling
- 🤖 Run multiple AI agent instances concurrently — because one agent is never enough
- 🎯 Believe AI agents are the future of software development

</details>
